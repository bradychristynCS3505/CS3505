...
*/
	...
	if(haveWePrintedMPFF ==0){
		printf("*** CS 3505:  Executing in (mpff_decode_from in libavcodec/mpffdec.c) ***\n*** CS 3505:  Modified by (Brady Mathews and Christyn Phillippi) ***\n");
		haveWePrintedMPFF = haveWePrintedMPFF + 1;
	}
    ...
    case  12: // OS/2 v1
        ...
        break;
    default:
        av_log(avctx, AV_LOG_ERROR, "unsupported MPFF file, patch welcome\n");
        return AVERROR_PATCHWELCOME;
    }

    /* planes */
    if (bytestream_get_le16(&buf) != 1) {
        av_log(avctx, AV_LOG_ERROR, "invalid MPFF header\n");
        return AVERROR_INVALIDDATA;
    }
    ...
    if (comp != MPFF_RGB && comp != MPFF_BITFIELDS && comp != MPFF_RLE4 &&
        comp != MPFF_RLE8) {
        av_log(avctx, AV_LOG_ERROR, "BMP coding %d not supported\n", comp);
        return AVERROR_INVALIDDATA;
    }

    if (comp == MPFF_BITFIELDS) {
        buf += 20;
    ...
    if (n * avctx->height > dsize && comp != MPFF_RLE4 && comp != MPFF_RLE8) {
    ...
    if (comp == MPFF_RLE4 || comp == MPFF_RLE8)
